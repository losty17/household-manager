...
export default function ShoppingList() {
  ...
  const singleBuyMutation = useMutation({
    mutationFn: async (item: ShoppingListItem) => {
      await productsApi.restock(item.product_id, {
        new_stock: item.suggested_quantity,
      });
      return item;
    },
    onSuccess: (item) => {
  ...
  const priorityConfig = {
    ...
    2: {
      ...
      bg: "bg-yellow-50 border-yellow-200 dark:bg-yellow-950/40 dark:border-yellow-800",
    },
    3: {
      label: "A Vencer",
          ...
          >
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <button
                ...
                disabled={
                  singleBuyMutation.isPending ||
                  boughtItems.has(item.product_id)
                }
                className={`flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center transition-colors ${boughtItems.has(item.product_id)
  ...
  const renderPredictSection = (
    sectionItems: PredictedShoppingListItem[],
    priority: 1 | 2 | 3 | 4,
  ) => {
    if (sectionItems.length === 0) return null;
    const config = priorityConfig[priority];
    ...
    <div className="p-4 pb-28 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => (mode === "current" ? refetch() : refetchPredict())}
        >
      ...
      <div className="flex rounded-lg border overflow-hidden">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium transition-colors ${mode === "current"
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium transition-colors ${mode === "predict"
              ? "bg-primary text-primary-foreground"
              : "bg-background text-muted-foreground hover:bg-muted"
            }`}
        >
        ...
        <div className="flex gap-2">
          {PREDICT_PERIODS.map((period) => (
            <button
              ...
              className={`flex-1 py-1.5 rounded-md text-sm font-medium border transition-colors ${predictDays === period.days
